...
function demo3TamperDetection() {
    ...
    blockchain.tamperWithBlock(2, {
        miner: 'Hacker',
        transactions: [Transaction_1.Transaction.createCoinbase('Hacker', 1000)]
    });
...
function demo4ForkResolution() {
    printHeader('DEMO 4: Fork Resolution (Longest Chain Rule)');
    const genesisChain = new Blockchain_1.Blockchain(2, 50);
    const nodeA = new Node_1.Node('Node A', genesisChain.clone());
    const nodeB = new Node_1.Node('Node B', genesisChain.clone());
    console.log('Both nodes start with genesis block only');
    const prevHash = genesisChain.getLatestBlock().hash;
    const block1A = new Block_1.Block(1, { miner: 'Miner A', transactions: [Transaction_1.Transaction.createCoinbase('Miner A', 50)] }, prevHash);
    block1A.mineBlock(2);
    const block1B = new Block_1.Block(1, { miner: 'Miner B', transactions: [Transaction_1.Transaction.createCoinbase('Miner B', 50)] }, prevHash);
    block1B.mineBlock(2);
    (0, Node_1.createForkScenario)(nodeA, nodeB, block1A, block1B);
    // Node A mines another block, making its chain longer
    console.log(`\nNode A mines next block on top of its chain...`);
    const block2A = new Block_1.Block(2, { miner: 'Miner A', transactions: [Transaction_1.Transaction.createCoinbase('Miner A', 50)] }, block1A.hash);
    block2A.mineBlock(2);
...
function bonusDifficultyComparison() {
...
function bonusTransactionLedger() {
    printHeader('BONUS: Complete Transaction Ledger');
    const blockchain = new Blockchain_1.Blockchain(2, 50);
    blockchain.mineBlock('Genesis-Miner');
    blockchain.addTransaction(new Transaction_1.Transaction('Genesis-Miner', 'Alice', 10));
    blockchain.addTransaction(new Transaction_1.Transaction('Genesis-Miner', 'Bob', 5));
    blockchain.mineBlock('Miner-1');
    blockchain.addTransaction(new Transaction_1.Transaction('Alice', 'Charlie', 3));
    blockchain.addTransaction(new Transaction_1.Transaction('Bob', 'Alice', 2));
